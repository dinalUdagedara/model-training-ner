...
| **UC-03** | Generate role-specific questions | Preconditions: résumé (and typically **job context**) available → session created → **next question** from QA agent when LLM enabled. **Partial:** needs `SESSION_QA_AGENT_ENABLED` + `OPENAI_API_KEY`; question count is agent behaviour — document in Ch 7. |
| **UC-04** | Practice with semantic feedback | Chat session → user messages → **evaluate answer** when agent enabled → feedback/score stored with thread. **Partial:** follow-ups and rubric depend on agent implementation. |
